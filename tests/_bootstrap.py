...
`import demand_planning_diagnostics` works with no environment setup
required. This keeps `python3 -m unittest discover -s tests -v` runnable
with zero preconditions, while the Makefile's `PYTHONPATH=src` continues
to serve the `demo`/`dashboard` CLI targets consistently.
"""
